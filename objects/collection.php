...
class Collection {
   ...
   //create database connection  when this class instantiated
    ...
    public function collectionCallSummaryExport($startdate,$enddate,$tagname){
        $currentdate = date('Y-m-d');
    ...
    public function collectionAgentCallDetails($extension,$username,$startdate,$enddate,$tagname){
     ...
     public function collectionCallAgentDetailsExport($extension,$username,$startdate,$enddate,$tagname){
     ...
     public function putCollectionCallComment($startimestamp, $getdate, $caller, $comment,$tag) {
       ...
       $query = "UPDATE `collectionteam_callsummary` SET `comment`='$comment',`tag`='$tag' WHERE `StartTimeStamp`='$startimestamp' AND `getDate`='$getdate' AND `Caller`='$caller'";
        //prepare query
        $stmnt = $this->conn->prepare($query);
