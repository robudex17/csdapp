...
function addcsdAgent() {
    ...
    .then(response => {
        ...
        var event_data = {};
        event_data.action = 'ADD CSD AGENT';
        event_data.performed_by = user;
        event_data.description = `New CSD Agent name:${data.name} extension:${data.extension} email:${data.email} was added by ${user}`
           ...
           var params = {};
            ...
            // alert(JSON.stringify(params))
             ...
             .then(response => {
                ...
                var event_data = {};
                event_data.action = 'Update CSD AGENT';
                event_data.performed_by = user;
                event_data.description = `Updated CSD Agent to name:${params.name} extension:${params.extension} email:${params.email}  by ${user}`
                fetch(`${HTTPADDR}api/create_event_log.php`, { method: 'post', body:JSON.stringify(event_data)})
                .then(() => {
                     alert(message);
                     location.reload();
            ...
            if(confirm(`Are you sure you want delete ${this.id} Agent? Deleting Agent will automatically delete Agent Records as well`)){
             ...
             .then(response => {
                 return response.json()
                  ...
                  //create data to send to log
                    var event_data = {};
                    event_data.action = 'DELETE CSD AGENT';
                    event_data.performed_by = user;
                    event_data.description = `DELETE CSD Agent with id/extension of ${params.extension} by ${user}`
                    fetch(`${HTTPADDR}api/create_event_log.php`, { method: 'post', body:JSON.stringify(event_data)})
                    .then(() => {
                         alert(message);
                         location.reload();
