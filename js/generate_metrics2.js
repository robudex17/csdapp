   ...
   fetch(url).then(response => {
    ...
    return res.json()
      ...
      for(var i= 0; i<array_tags.length ;i++){ 
       ...
       data_array = [data_range,'',grand_total_tag ];
       jsonobj.tableData[0].data.push([])
       jsonobj.tableData[0].data.push(data_array)
...
function getMetricsTagTable(res){
  ...
  document.getElementById('options-tag').textContent = `${response.option.toString().toUpperCase()} TAGS`;
   ...
   for (const [key, value] of Object.entries(response)){
    ...
    document.getElementById('date_time_range_tag').textContent = response.dateRange;
    document.getElementById('grand_total_tag').textContent = response.total_records;
