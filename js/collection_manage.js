...
function addCollectionAgent() {
    ...
    fetch(`${HTTPADDR}api/collection/create_collection_agent_api.php`, { method: 'post', body:JSON.stringify(data)} )
    .then(response => {
        ...
        var event_data = {};
        event_data.action = 'ADD COLLECTION AGENT';
        event_data.performed_by = user;
        event_data.description = `New COLLECTION Agent name:${data.name} extension:${data.extension} email:${data.email} was added by ${user}`
...
function getAllCollectionAgents() {
	...
	fetch(`${HTTPADDR}api/collection/collection_get_all_collection_agent_api.php`)
	.then(data => {
		data = data.json();
		return data
	}).then(data =>
        ...
        updateBtn.addEventListener('click', function(e){
           ...
           var params = {};
            ...
            // alert(JSON.stringify(params))
             ...
             fetch(`${HTTPADDR}/api/collection/update_collection_agent_api.php`, {method:'post', body:JSON.stringify(params)})
             .then(response => {
                 return response.json()
             }).then(data => {
                message = JSON.stringify(data.message);
                var event_data = {};
                event_data.action = 'Update COLLECTION AGENT';
                event_data.performed_by = user;
                event_data.description = `Updated COLLECTION Agent to name:${params.name} extension:${params.extension} email:${params.email}  by ${user}`
                fetch(`${HTTPADDR}api/create_event_log.php`, { method: 'post', body:JSON.stringify(event_data)})
                .then(() => {
                     alert(message);
                     location.reload();
       ...
       // actDelete.style.margin = "4px";
        ...
        actDelete.addEventListener('click',function(e){
            ...
            if(confirm(`Are you sure you want delete ${this.id} Agent? Deleting Agent will automatically delete Agent Records as well `)){
                 fetch(`${HTTPADDR}api/collection/delete_collection_agent_api.php`, {method:'post', body:JSON.stringify(params)})
             .then(response => {
                 return response.json()
             }).then(data => {
                  ...
                  //create data to send to log
                    var event_data = {};
                    event_data.action = 'DELETE COLLECTION AGENT';
                    event_data.performed_by = user;
                    event_data.description = `DELETE COLLECTION Agent with id/extension of ${params.extension} by ${user}`
                    fetch(`${HTTPADDR}api/create_event_log.php`, { method: 'post', body:JSON.stringify(event_data)})
                    .then(() => {
                         alert(message);
                         location.reload();
